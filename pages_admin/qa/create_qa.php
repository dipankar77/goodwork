<?php 
$question = $answer = $skill_category = ""; // Define variables and initialize with empty values
$skill_question_err = $skill_answer_err = $skill_category_err  = "";
if($_SERVER["REQUEST_METHOD"] == "POST"){ echo ""; print_r($_POST); // Processing form data when form is submitted
    ...
    $input_skill_question = trim($_POST["skill_question"]); // Validate menu name
    if(empty($input_skill_question)){
        $skill_question_err = "Please enter your Question.";
    } else{
        $question = $input_skill_question;
    }
    ...
    if(empty($skill_category_err) || empty($skill_question_err) || empty($skill_answer_err)){ // Check input errors before inserting in database
        $sql = "INSERT INTO qa (question, answer, pageid) VALUES (?, ?, ?)"; // Prepare an insert statement
        if($stmt = mysqli_prepare($link, $sql)){
            mysqli_stmt_bind_param($stmt, "ssi", $param_question, $param_answer, $param_skill_category); // Bind variables to the prepared statement as parameters
            $param_question = $question; // Set parameters
            $param_answer = $answer; // Set parameters
            $param_skill_category = $skill_category; // Set parameters            
            if(mysqli_stmt_execute($stmt)){ // Attempt to execute the prepared statement
                header("location: admin.php?page=qa"); // Records created successfully. Redirect to landing page
                exit();
            } else{
...
<div class="row">
    <div class="col-md-12">
        ...
        <select class="form-select" name="skillcategory" aria-label="Default select example">
            ...
            <?php
            $sql = "SELECT id, category_name FROM categories WHERE cattype = 1";// Attempt select query execution
            ...
            <div class="form-group <?php echo (!empty($skill_question_err)) ? 'has-error' : ''; ?>">
                <label>Question</label>
                <input type="text" name="skill_question" class="form-control">
                <span class="help-block"><?php echo $skill_question_err;?></span>
            </div>
            ...
            <a href="admin.php?page=qa" class="btn btn-default">Cancel</a>
        </form>
    </div>
</div>        
